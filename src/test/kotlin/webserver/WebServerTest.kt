...
class WebServerTest {
  ...
  val route = configureRoutes(handlers)
...
// ***** Tests for Handlers *****
  ...
  @Test
  fun `can process multiple params and no name`() {
    val request = Request("http://www.imperial.ac.uk/say-hello?style=shouting")
    assertEquals("HELLO, WORLD!", helloHandler(request).body)
  }
...
// *** More flexible routing ***
  ...
  fun `calling configureRoutes() returns app which can handle requests`() {
    ...
    val app = configureRoutes(handlers)
...
//  *** Filters ***
  ...
  fun `filter prevents access to protected resources `() {
    ...
    val app1 = configureRoutes(handlers)
    val app  = requireToken(app1)
